...
const PageManageUsers = () => {
  ...
  const containerVar = {
    ...
    visible: { opacity: 1, scale: 1, transition: { duration: 0.4, type: "spring", stiffness: 200, damping: 10 } },
    exit: { opacity: 0, scale: 0.8 }
  }

  const viewUsersVar = {
    hidden: { opacity: 0, },
    visible: { opacity: 1, y: isAdding ? 5 : 20, transition: { duration: 0.8 } },
  }
  return (
    <div className='flex overflow-hidden'>
      ...
      <div className="w-full">
        ...
        <div className="mt-4 flex justify-end px-8">
          ...
          >
            {isAdding ? <IoClose className='text-lg' /> : <FaPlus className='text-lg' />}
            {isAdding ? "Close form" : "Add user"}
          </button>
        </div>
        <AnimatePresence>
          {isAdding && (
            <motion.div variants={containerVar} initial="hidden" animate="visible" exit="exit" className="overflow-hidden">
              <AddUser />
            </motion.div>
          )}
        </AnimatePresence>
